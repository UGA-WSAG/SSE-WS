...
import mepcotterell.util.SimpleCache;
...
public class OntologyManager {
    ...
    private IRI str2iri (String str) {
    ...
    public OntologyManager(String filename) throws IOException,
        ...
        try {
            ...
            ontology = manager.loadOntology(str2iri(path));
            
            log.info("Successfully loaded the primary ontology");
            
            ontologyInstances.put(ontology.getOntologyID().getOntologyIRI().toString(), ontology);
         
            // load the import ontology from web
            Set<OWLOntology> importsOnts = ontology.getImports();
            for (OWLOntology o: importsOnts) {
                
                log.info(String.format("Loading import from IRI = %s", o.getOntologyID().getOntologyIRI().toString()));
                
                String opath = o.getOntologyID().getOntologyIRI().toString();
                if (!manager.contains(o.getOntologyID())) {
                    OWLOntology importOnt = manager.loadOntology(str2iri(opath));
                    ontologyInstances.put(opath, importOnt);
                }
